...
void image_filter_response(int fd, const ReqData *reqData) {
    // IMPLEMENT THIS
    char filter_path[strlen(FILTER_DIR) + 25];
    memset(filter_path, '\0', strlen(FILTER_DIR) + 25); // ensures that filter_path is null terminated
    strcpy(filter_path, FILTER_DIR);

    char image_path[strlen(IMAGE_DIR) + 25];
    memset(image_path, '\0', strlen(IMAGE_DIR) + 25); // ensures that image_path is null terminated
    strcpy(image_path, IMAGE_DIR);

    // 1. check if request is valid
    char *method = reqData->method;
    char *path = reqData->path;
    if ((strcmp(method, "GET") == 0) && (strcmp(path, "/image-filter") == 0)) {
        // check to make sure the query parameters are valid
        // set filter and image to 1 if both parameters are present and valid
        int filter = 0;
        int image = 0;
        // iterate through query parameters
        int num_params = sizeof(reqData->params) / sizeof(reqData->params[0]);
        for (int i = 0; i < num_params; i++) {
            // check if image and filter parameters are present
            if (strcmp(reqData->params[i].name, "image") == 0) {
                // check if image filename is valid (doesn't contain a slash)
                if (strchr(reqData->params[i].value, '/') != NULL) {
                    bad_request_response(fd, "invalid character in image name");
                    exit(1);
                }
                // check if image file exists within /a4/images/ directory
                else {
                    strcat(image_path, reqData->params[i].value);
                    int valid_image = access(image_path, R_OK);
                    if (valid_image < 0) {
                        bad_request_response(fd, "The requested image does not exist.");
                        exit(1);
                    } 
                    else {
                        image = 1;
                ...
                // check if filter is valid (doesn't contain a slash)
                if (strchr(reqData->params[i].value, '/') != NULL) {
                    bad_request_response(fd, "invalid character in filter name");
                    exit(1);
                }
                // check if filter file exists within /a4/filters/ directory
                else {
                    strcat(filter_path, reqData->params[i].value);
                    int valid_filter = access(filter_path, X_OK);
                    if (valid_filter < 0) {
                        bad_request_response(fd, "The requested filter does not exist.");
                        exit(1);
                    } 
                    else {
                        filter = 1;
